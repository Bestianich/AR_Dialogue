...
public class MemoryDataDrawer : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        switch (dataType)
        {
            ...
            case DataType.IMAGE:
                EditorGUI.BeginChangeCheck();
                ObjectWrapper currentImage = _value.managedReferenceValue as ObjectWrapper;
                if(currentImage == null)
                    currentImage = new ObjectWrapper();
                Image newImage = EditorGUI.ObjectField(valueRect, currentImage.Target, typeof(Image), true) as Image;
                Debug.Log(newImage);
                if (EditorGUI.EndChangeCheck())
                {
                    if (newImage != currentImage.Target)
                    {
                        _value.managedReferenceValue = new ObjectWrapper(newImage);
                        _value.serializedObject.ApplyModifiedProperties();
                        _value.serializedObject.Update();
                        _value = property.FindPropertyRelative("Value");
                        changed = true;
